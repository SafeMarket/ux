...
module.exports = function StoreProductsController($scope, transactionManager, growl, $state) {
  ...
  $scope.createOrder = function createOrder() {
    ...
    const transactionRequest = solWrappers.OrderReg.getTransactionRequest(
      'create(address,uint256,address,uint256[],uint256[],uint256)', inputs, {
        value: $scope.total.to('WEI')
      }
    )
    transactionManager.request('Create a New Order', transactionRequest).then((transactionReceipt) => {
      const eventLogs = solWrappers.OrderReg.parseTransactionReceipt(transactionReceipt)
      const orderId = eventLogs[0].topics.id
      growl.success(`Order #${orderId.to('number')} Created!`)
      $state.go('order', {
        orderRegAddressHexPrefixed: solWrappers.OrderReg.address.to('hex.prefixed'),
        orderIdNumber: orderId.to('number')
      })
    })
  }
}
