...
module.exports = function ordersDirective($state) {
  return {
    scope: {
      orderReg: '=',
      ordersBy: '='
    ...
    link: function ordersLink($scope) {
      ...
      $scope.getHref = function getHref(order) {
        return $state.href('order', {
          orderRegAddressHexPrefixed: $scope.orderReg.address.to('hex.prefixed'),
          orderIdNumber: order.id.to('number')
        }, { absolute: true })
      }
    }
  }
