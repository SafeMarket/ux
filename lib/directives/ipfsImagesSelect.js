...
module.exports = function countrySelectDirective(growl) {
  return {
      ...
      $scope.$watch('file', () => {
        ...
        fileReader.onload = (progressEvent) => {
          ...
          if (imageDataUrl.indexOf(DATA_URL_JPEG) !== 0) {
            growl.error(getBlurb('image_type_error'))
            return
          }
          const image = new Amorph(imageDataUrl.substr(DATA_URL_JPEG.length), 'base64')
          $scope.images.push(image)
          growl.info('Adding 1 file to IPFS')
          ipfs.addFile(image).then((multihash) => {
            growl.success(`Added ${multihash} to IPFS`)
          })
