...
Order.prototype.download = function download() {
  ...
  return Q.all([
    ...
    this.buyerPayoutMicroperun = this.storePayoutMicroperun.as('bignumber', (bignumber) => {
...
Order.prototype.calculate = function calculate() {
  const prebufferWEI = this.prebuffer.to('WEI')
  this.bufferRemaining = new Pricemorph(this.receivedWEI.as('bignumber', (bignumber) => {
    return bignumber.minus(prebufferWEI.to('bignumber'))
  }), 'WEI')
  this.bufferRemainingPerun = this.bufferRemaining.to('WEI').as('bignumber', (bignumber) => {
    return bignumber.div(prebufferWEI.to('bignumber'))
  })
  this.storePayout = new Pricemorph(prebufferWEI.as('bignumber', (bignumber) => {
    return bignumber.times(this.storePayoutMicroperun.to('bignumber')).times(MICRO)
  }), 'WEI')
  this.buyerPayout = new Pricemorph(
    this.received.to('WEI').as('bignumber', (bignumber) => {
      return bignumber.minus(this.storePayout.to('WEI').to('bignumber'))
    }),
  'WEI')
}
