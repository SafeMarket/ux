...
const compressMultihash = require('../utils/compressMultihash')
...
function Store(address) {
  ...
  if (address) {
    ...
    if (settings.profile) {
      getIsOwner(address, settings.profile.persona.address).then((isOwner) => {
...
Store.prototype.download = function download() {
  ...
  return this.schema.download((method, args) => {
    return this.solWrapper.fetch(method, args).then((value) => {
      return value
    })
  }).then(() => {
    this.setDefaults()
    return this.setFromChainState()
  }).finally((err) => {
    this.isReady = true
  })
}
...
Store.prototype.setDefaults = function setDefaults() {
  ...
  this.coinbase = settings.profile ? settings.profile.persona.address.clone() : new Amorph(0, 'number')
...
Store.prototype.setFromChainState = function setFromChainState() {
  ...
  this.coinbase = state.coinbase.clone()
  ...
  this.metaMultihash = uncompressMultihash(state.metaMultihash)
  this.approvedArbitrators = state.approvedArbitrators.map((address) => {
...
Store.prototype.approvedArbitrators = function approvedArbitrators() {
  this.approvedArbitrators.push(new Amorph('', 'ascii'))
}
...
Store.prototype.getLocalState = function getLocalState() {
  return {
    ...
    metaMultihash: compressMultihash(this.getMetaMultihash()),
    products: this.products.map((product) => {
    ...
    approvedArbitrators: this.approvedArbitrators.map((address) => {
      return address.clone()
    })
  }
}
